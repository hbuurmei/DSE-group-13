...
const t0 = 72 * 100 * 60
const dt = 6
const distance_sc = 50e3  # Orbital altitude of spacecraft above collision altitude
...
function run_sim(;plotResults=true)
    ...
    while (debris_counter / tot_debris_n < target_fraction)
        ...
        if mod(round(t), 1000) == 0  # Smoothest visualisation is for dt
            println("--------------------------------------------")
    ...
    return (ts, percentages, increased_a_percentage, avoidance_counter)
end

@time (times, perc, perc_increased_a, n_avoidance) = run_sim(plotResults=false)

time_required = times[end]/(3600*24)  # In days

println("The number of potential collisions (< 100 m) for this reference case: ", n_avoidance)
